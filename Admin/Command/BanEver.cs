...
namespace BotCore
{
	internal class BanEver : AbsCommand, IStandartCommand, ISplitName
	{
		public override System.String Name { set; get; } = CommandText.BanEver;
		...
		{
			IStandartCommand standartCommand = new BanEver();
			ISplitName splitName = new BanEver();
		...
		public void SendMessage(TelegramBotClient botClient)
		{
			...
			if (user.IsAdmin == 3)
			{
				botClient.EditMessage(_message.From.Id, user.MessageID, "Пользователь забанен на навсегда!", "36 - LimitedUser", replyMarkup: inlineButton.InteractionUsers(userTwo, db._featuredUserNews.Any(p => p.UserId == userTwo.ID && p.UserWhoAddedId == user.ID), db.GetFeaturedUsers(user, userTwo), isAdmin: true));
			}
