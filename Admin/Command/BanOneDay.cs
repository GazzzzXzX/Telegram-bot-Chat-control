...
namespace BotCore
{
	internal class BanOneDay : AbsCommand, IStandartCommand, ISplitName
	{
		public override System.String Name { set; get; } = CommandText.BanOneDay;
		...
		{
			IStandartCommand standartCommand = new BanOneDay();
			ISplitName splitName = new BanOneDay();
		...
		public void SendMessage(TelegramBotClient botClient)
		{
			...
			if (user.IsAdmin == 3)
			{
				botClient.EditMessage(_message.From.Id, user.MessageID, "Пользователь забанен на 1 день!", "36 - LimitedUser", replyMarkup: inlineButton.InteractionUsers(userTwo, db._featuredUserNews.Any(p => p.UserId == userTwo.ID && p.UserWhoAddedId == user.ID), db.GetFeaturedUsers(user, userTwo), isAdmin: true));
			}
