...
namespace BotCore
{
	internal class MuteOneDay : AbsCommand, IStandartCommand, ISplitName
	{
		public override System.String Name { set; get; } = CommandText.MuteOneDay;
		...
		{
			IStandartCommand standartCommand = new MuteOneDay();
			ISplitName splitName = new MuteOneDay();
		...
		{
			userTwo.BanDate = DateTime.Now.AddDays(1);
		...
		public void SendMessage(TelegramBotClient botClient)
		{
			...
			if (user.IsAdmin == 3)
			{
				botClient.EditMessage(_message.From.Id, user.MessageID, "Пользователь замучен на 1 день!", "36 - LimitedUser", replyMarkup: inlineButton.InteractionUsers(userTwo, db._featuredUserNews.Any(p => p.UserId == userTwo.ID && p.UserWhoAddedId == user.ID), db.GetFeaturedUsers(user, userTwo), isAdmin: true));
			}
