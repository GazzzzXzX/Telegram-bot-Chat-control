...
namespace BotCore
{
	internal class PublishChatNotificationYes: AbsCommand, IStandartCommand
	{
		public override System.String Name { set; get; } = CommandText.PublishChatNotificationYes;
						...
						if (picture == null)
						{
							try
							{
								var mes = await botClient.SendTextMessageAsync(ThisUser.IDChannel, TextAndButton.Text.Text,
								                                     replyMarkup: temp);
								await botClient.PinChatMessageAsync(ThisUser.IDChannel, mes.MessageId, true);
							}
							catch (Exception e)
							{
